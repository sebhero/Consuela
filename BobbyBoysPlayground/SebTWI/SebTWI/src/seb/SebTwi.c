...
// original author Arduino, implemented to use with TWI
//
...
void TWI_SetClock(Twi* pTwi, uint32_t dwTwCk, uint32_t dwMCk) {
    ...
    if (dwCkDiv < 8)
    {
        //error not valid clock speed
    }

    pTwi->TWI_CWGR = 0;
    pTwi->TWI_CWGR=(dwCkDiv<<16)|(dwCkDiv<<8) | dwClDiv;
}
...
void TWI_Disable(Twi *pTwi){
    ...
    //pTwi slave mode disabled, TWI maste mode disabled
 ...
 */
void TWI_EnabledIt(Twi *pTwi, uint32_t sources)
{
    //assert(pTwi != NULL);
